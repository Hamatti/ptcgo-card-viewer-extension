...
describe("List parsing", function () {
  it("should remove leading zero if one exists", () => {
    assert.equal(getSubsettedNumber("XY", "001"), "1");
    assert.equal(getSubsettedNumber("CRZ", "012"), "12");
  });

  it("should not do anything if no leading zeroes", () => {
    assert.equal(getSubsettedNumber("CRZ", "112"), "112");
  });
});
